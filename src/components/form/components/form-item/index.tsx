...
import { useFieldContext, type SearchTransformKeyFn } from '../../field-context'
...
export const FormItemContextKey: InjectionKey<FormItemContextValue> = Symbol('FormItemContext')
  ...
  setup(props, { attrs }) {
    ...
    const renderContent = () => {
      ...
      const processedChildren = children.map((child, index) => {
        ...
        // 检查是否是 ProForm 组件
        const isProFormComponent = (child.type as any)?.displayName === 'ProFormComponent'
      ...
      if (needLightWrapper.value) {
        ...
        wrappedChildren = (
          <LightWrapper {...props.lightProps} label={convertedLabel as string | (() => VNode)}>
            {processedChildren}
          </LightWrapper>
        )
      }
    ...
    const finalRules = computed(() => {
      ...
      if (props.required) {
        const hasRequiredRule = rules.some((rule: any) => rule && rule.required === true)
        if (!hasRequiredRule) {
          rules.unshift({
            required: true,
    ...
    return () => {
      ...
      if (typeof children?.[0] === 'function') {
        return (
          <FormItem
            ...
            help={typeof props.help === 'function' ? undefined : (props.help as string)}
      ...
      const formItemName = computed(() => {
        ...
        return Array.isArray(finalName.value) ? finalName.value.join('.') : finalName.value
      })
