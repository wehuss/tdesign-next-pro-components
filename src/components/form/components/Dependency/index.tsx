...
export interface ProFormDependencyProps {
  ...
  /** 子元素渲染函数 */
  children: (values: Record<string, any>, form: ProFormInstance) => VNode | VNode[]
}
  ...
  setup(props, { slots }) {
    ...
    const flattenNames = computed(() => {
      return props.name.map((itemName) => {
        const name: (string | number)[] = Array.isArray(itemName) ? [...itemName] : [itemName]
    ...
    const dependencyValues = computed(() => {
      ...
      for (let i = 0; i < props.name.length; i++) {
        ...
        if (typeof form.getFieldFormatValueObject === 'function') {
          ...
          if (value && Object.keys(value).length) {
            ...
            if (itemValue !== undefined) {
              const originPath = Array.isArray(originName) ? originName : [originName]
        ...
        const fieldPath = Array.isArray(itemName) ? itemName.join('.') : itemName
