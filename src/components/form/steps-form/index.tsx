...
// 注入 key
const StepsFormContextKey: InjectionKey<StepsFormContextValue> =
  Symbol("StepsFormContext");
const StepFormContextKey: InjectionKey<StepFormProps> =
  Symbol("StepFormContext");
...
export const StepsForm = defineComponent({
  ...
  setup(props, { slots, emit, expose }) {
    ...
    const lastStep = computed(
      () => currentStep.value === formArray.value.length - 1
    );
    ...
    return () => {
          ...
          {slots.default?.().map((item: any, index: number) => {
            ...
            return (
              <div
                class={[
                  "pro-steps-form-step",
                  { "pro-steps-form-step-active": isShow },
                ]}
                key={name}
      ...
      if (props.stepsFormRender) {
        ...
        stepsFormDom = (props.stepsFormRender as any)(
          layoutContent,
          <Space>{submitterDom}</Space>
        );
